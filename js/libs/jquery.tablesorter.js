...
!(function($) {
					...
					if (!$(b[k]).hasClass(tc.cssInfoBlock)) {
						...
						for (i = 0; i < totalRows; ++i) {
							...
							for (j = 0; j < totalCells; ++j) {
								...
								//console.info(parsers[j],parsers[j].format(t, table, c[0].cells[j], j))
	...
	ts.addWidget({ 
		    ...
		    format: function(table) { 
		    	$(table).find('tbody tr:odd td').css('background-color','#fff');
		    	$(table).find('tbody tr:even td').css('background-color','#DBDFE2');	
		    },
